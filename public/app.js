...
function renderSubs(subs) {
  $('#subsTable tbody').innerHTML = subs.map((s) => {
    ...
    </tr>${s.machine ? `<tr class="machine-row" data-for="${s.sub_id}" hidden><td></td><td colspan="10" class="machine-cell"><span class="pill machine-pill">BD auto</span> ${((s.machine.file_bytes || 0) / 1024).toFixed(1)} KB · <a href="/api/file/${s.machine.sub_id}">stáhnout</a></td></tr>` : ''}`;
